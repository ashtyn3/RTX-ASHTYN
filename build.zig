...
fn int(value: ?[]const u8, def: []const u8) u64 {
    return std.fmt.parseInt(u64, value orelse def, 10) catch {
        @panic("Failed to parse env");
    };
}
...
pub fn build(b: *std.Build) void {
    ...
    // const mem_len = std.fmt.parseInt(u64, env_map.get("SHARED_MEM_LEN") orelse "1500", 10) catch {
    //     @panic("Failed to parse env");
    // };
